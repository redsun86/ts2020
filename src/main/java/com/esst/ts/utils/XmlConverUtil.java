...
public class XmlConverUtil {
	...
	public static Map<String, Object> getPageInfo(String xml) {
		if (StringUtils.isEmpty(xml)) {
			return null;
		}
		return readDocumentGetPageInfo(xml);
	}
	...
	public static Map<String, Object> readDocumentGetPageInfo(String xml) {
		Map<String, Object> pageInfo = new HashMap<>();
		List<Element> sonElement = getDocument(xml).getRootElement().elements();
		...
		for (Element element : sonElement) {
			if (element.getName().equals("Method")) {
				...
				if (value.elements().size() > 2) {
					rows = value.elements();
					List<Element> pageInfoEle = rows.get(0).elements();
					pageInfo.put("total", pageInfoEle.get(1).getText());
				}
			}
		}
		return pageInfo;
